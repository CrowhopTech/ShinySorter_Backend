...
func (mc *mongoConnection) ListQuestions(ctx context.Context) ([]*imagedb.Question, error) {
	cursor, err := mc.questionsCollection.Find(ctx, bson.M{})
	if err != nil {
		return nil, fmt.Errorf("error while running Find: %v", err)
	}

	results := []*imagedb.Question{}

	for cursor.Next(ctx) {
		var result imagedb.Question
...
func (mc *mongoConnection) CreateQuestion(ctx context.Context, q *imagedb.Question) (*imagedb.Question, error) {
	...
	createdQuestion := imagedb.Question{}
...
func (mc *mongoConnection) ModifyQuestion(ctx context.Context, q *imagedb.Question) (*imagedb.Question, error) {
	trueVal := true
	...
	updatedQuestion := imagedb.Question{}
